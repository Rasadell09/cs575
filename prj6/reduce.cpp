...
main(int argc, char *argv[])
{
	...
	for (int num_elements = 1024; num_elements <= 8388608; num_elements *= 2) {
	//int num_elements = 512;
		...
		int num_work_groups = num_elements / LOCAL_SIZE;
		...
		float sum = 0;
		for (int x = 0; x < num_work_groups; x++) {
			sum += hC[x];
		}
		...
		fprintf(stderr, "%8d\t%4d\t%10d\t%10.3lf MegaMult-ReductionPerSecond\n",
